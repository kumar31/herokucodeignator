...
$(document).ready(function() {
	...
	$('#results').load("<?php echo base_url();?>index.php/client_proposals_hired/getblogdata/<?php echo $event_detail[0]['event_id']; ?>", {'group_no':track_load}, function() {track_load++;}); //load first group
	...
	$(window).scroll(function() { //detect page scroll
		...
		if($(window).scrollTop() + $(window).height() == $(document).height())  //user scrolled to bottom of the page?
		{
			...
			if(track_load <= total_groups && loading==false) //there's more data to load
			{
				...
				$.post('<?php echo base_url();?>index.php/client_proposals_hired/getblogdata/<?php echo $event_detail[0]['event_id']; ?>',{'group_no': track_load}, function(data){
