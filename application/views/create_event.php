		...
		<div class="col-md-4">
		 ...
		 <div class="form-group prepend-top">
		  ...
		  <br>
			...
			?>
			<div class="prepend-top clearfix" id="hideeventtype">
		  ...
		  <div class="prepend-top row">
			<div class="form-group clearfix">
			  
			</div>
			<div class="clearfix privacy-selector">
				<div class="col-sm-12">
					<label class="pull-left" for="">Talent Type
				  </label>
			    </div>			  
				<div class="col-sm-6 text-center">
				<img class="center-block" style="height: 250px;" src="<?php echo base_url();?>img/employees.png">
				<br>
				  <label>
...
<p>I HEREBY ACKNOWLEDGE THAT I HAVE READ AND UNDERSTAND THE FOREGOING TERMS OF SERVICE AND PRIVACY POLICY AND AGREE THAT MY USE OF THE SERVICE IS AN ACKNOWLEDGMENT OF MY AGREEMENT TO BE BOUND BY THE TERMS AND CONDITIONS OF THIS AGREEMENT.
  ...
  <script>
		...
		function register(){
			...
			var talent_type = $("input[name='talent_type']:checked").val();
			...
			if(typeof talent_type === "undefined") {
				var talent_type = ''; 
			}
			else {
				var talent_type = $("input[name='talent_type']:checked").val();
	...
	$('#waitersnum').on('change', function() {
		var type = 1;
		var start_datetime = $("#startdate").val(); //alert(start_datetime); die;
		var end_datetime = $("#enddate").val();
		add(type,start_datetime,end_datetime); // or $(this).val()
		
	});
	
	
	$("input[type=radio][name=talent_type]").change(function() {
	...
	function add(type){
		var talenttype = type; 
		var talentcount = $( "#waitersnum option:selected" ).text(); 
		var start_datetime = $("#startdate").val(); //alert(start_datetime); die;
		var end_datetime = $("#enddate").val();
		
			var url = '<?php echo $webserviceurl; ?>index.php/appropriate';
			
			$.ajax({
				'type' : 'POST',
				'url': url,
				'data': {'talentcount':talentcount,'talenttype':talenttype,'start_datetime':start_datetime,'end_datetime':end_datetime},
				//'dataType': 'json',
				success: function(data) {
					var alertmessage = JSON.stringify(data['message']);
					var result = JSON.stringify(data['result']);
					$("#estimateamt").text(result);
