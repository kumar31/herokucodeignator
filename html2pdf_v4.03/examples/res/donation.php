...
$selectDb=mysql_select_db($database) ;
 ...
 $tax=$payshul=$_GET['payshul'];
 $quss=mysql_query("SELECT * FROM ProcessingFee WHERE Type='Donation'");
...
$query=mysql_query("SELECT * FROM `Donate` WHERE DonateID=$DonateID");
		
		while($donate = mysql_fetch_array($query)){
			
		$DonateType=	$donate['DonateType'];
		$BuyAmounts=	$donate['Amount'];
		$UserID=$donate['ShulID'];
		
		
		
		//$tax=$donate['PayforShul'];
		}
...
// Call the function to get the currency converted
 ...
 ?>
	...
	<table style="width: 50%; font-size: 16px;"  cellspacing="0">
		<tr>
			<td style="width: 100%">Invoice Id : <b>INVOICE <?php echo $DonateID; ?></b>  </td>
			...
			<tr style="font-weight: bold;">
                ...
                <td style="width: 20%; text-align: center; border: solid 1px #000">
                 <?php 
				   ...
				   $toAmounts= get_currency('USD', $mycurrency, $BuyAmounts);
					...
					$topays=($BuyAmounts*($percent/100))+$extrafee;
					$withoutopay=(($BuyAmounts*($percent/100))+$extrafee)/2;
