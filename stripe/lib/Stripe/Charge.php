...
class Stripe_Charge extends Stripe_ApiResource
{
  ...
  public function instanceUrl($charge)
  {
    $id = $charge;
    $class = get_class();
   // if (!$id) {
  ...
  public function refund($params=null)
  {
	...
	$requestor = new Stripe_ApiRequestor('sk_test_l8FVfiAVF7kbHq3oGOD2hCkL');
	$url = self::instanceUrl($charge) . '/refunds';
    //list($response, $apiKey) = $requestor->request('post', $url, $data);
   // $refund=$this->refreshFrom($response, $apiKey);
     $refund = $requestor->request('post', $url, $data);
    return $refund;
  }
