...
class reject_talent_by_client_model extends CI_Model {
	...
	function index()
	{
		...
		$data = array(
			'status' 					=> 4,
			'client_reject_reason' 		=> $_POST['client_reject_reason'],
			'datetime' 					=> $timeNdate
		);
		...
		$subject = "Outfit - Rejected for event"; 
		$message ="<p>Hi ".$to_name.",</p>";
		$message .="<p>You are rejected for ".$event_name." event.</p>";
		...
		$text = "You are rejected for ".$event_name." event.";
		$text = urlencode($text);
