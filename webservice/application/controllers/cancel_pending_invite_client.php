...
class cancel_pending_invite_client extends REST_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('cancel_pending_invite_client_model');
	...
	public function index_post()
	{
		$validationandresult = new validationandresult();
			if(isset($_POST) != "")
			{
					...
					// Empty key checking.
					$pre_key = array('event_id','talent_id','client_id');
					...
					else
					{
						...
						{ 
							...
							$result = $this->cancel_pending_invite_client_model->index();	
	...
	function talentidcheck(){
		...
		if(!empty($result)){
			...
			$this->form_validation->set_message('talentidcheck', 'Already rejected by client for this event');
			return false;			
		}
		else {
