...
class accept_event_by_talent extends REST_Controller {
			...
			if(isset($_POST) != "")
			{
					...
					else
					{
						...
						{ 
							...
							$result = $this->accept_event_by_talent_model->index();	
	...
	function talentidcheck(){
		...
		if(!empty($result)){
			...
			$this->form_validation->set_message('talentidcheck', 'Already accepted for this event');
			return false;			
		}
		else {
