...
class hire_by_client extends REST_Controller {
			...
			if(isset($_POST) != "")
			{
					...
					else
					{
						...
						{ 
							...
							$result = $this->hire_by_client_model->index();	
	...
	function talentidcheck(){
		...
		if(!empty($result)){
			...
			$this->form_validation->set_message('talentidcheck', 'Already hired by client for this event');
			return false;			
		}
		else {
